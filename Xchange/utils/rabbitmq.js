...
const { SendApproverMail,SharepriceApproverMail, UpdateApproverMail, CreateAllocationEntries, SendSellerMail, SendBuyerMail, SendTransferMail, SendUserMail, PopulateUserName } = require('./RABBITMQSUBSCRIBERS')
...
exports.Subscribe = async () => {
  ...
  this.RabbitMQService.consume('SEND_EMAIL_TO_APPROVERS_ON_SHAREPRICEUPDATE', (msg) => {
    const data = JSON.parse(msg.content.toString());
    const status = SharepriceApproverMail(data.shareprice)
    if (!status) {
      this.RabbitMQService.rejectMessage(msg, 1);
    }
    this.RabbitMQService.acknowledgeMessage(msg);
