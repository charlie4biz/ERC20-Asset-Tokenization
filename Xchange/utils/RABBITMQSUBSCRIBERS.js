...
exports.SharepriceApproverMail = async function(shareprice) {
    try {
        // Get All Approvers
        let approvers = []
        let operationStatus 
        ...
        if (approvers.length > 0) {
            ...
            const email = EmailDetails(`
                <div>
                    <h2><b>Please find the details of the newly updated share price below :</b></h2>
                    <h4><b>Share Price</b> : ${shareprice.price},</h4>
                </div>
                <h4>To Approve Proceed To <a href='${shareprice.platformURL}'>The OneExchange Portal</a> </h4>
            `)
            var EmailParams = {
                sourceEmail : Config.appEmail,
                body : email.body,
                subject : "Your Approval Is Needed! An Updated Share Price Created"
            }
...
exports.SendTransferMail = async function(trade) {
    try {
            ...
            let SoapClient1 = await soap.createClientAsync(Config.ADServiceURL) 
            let responseFromAD = await SoapClient1.GetInfo2Async({xusername : trade.receiver})
            const ADRes = await SerializeXML(responseFromAD[0].GetInfo2Result)
            const adProfile =  SerializeAD(ADRes.root.record[0])
