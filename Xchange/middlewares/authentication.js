...
/**
 * Ensure Resource is Accessible To Auditors
 */
exports.IsAuditor = async (req, res, next) => {
  try {
    if (req.authUser.userRole === UserModel.UserType.AUDITOR || req.authUser.userRole === UserModel.UserType.ADMIN || req.authUser.userRole === UserModel.UserType.SUPERADMIN) {
      next()
    } else {
      return next(response.PlainError(Errors.ACCESSDENIED, HttpStatus.UNAUTHORIZED, GetCodeMsg(Errors.ACCESSDENIED), {error : "This Resource Is Only Accessible By Auditors"}))
    }
  } catch (error) {
    return next(response.PlainError(Errors.AUTHORIZATIONERR, HttpStatus.UNAUTHORIZED, GetCodeMsg(Errors.AUTHORIZATIONERR), error))
  }
