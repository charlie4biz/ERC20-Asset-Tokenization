  ...
  /**
    ...
    async GetWalletTransactions(req, res, next) {
      ...
      try {
        ...
        const transactions = await TransactionModel.find({$and: [{$or : [{type : TransactionModel.Type.FUND},{type : TransactionModel.Type.WITHDRAW}]},{$or : [{from : req.authUser.username},{to : req.authUser.username}, {userId : req.authUser._id}]}]}).skip((!offset)?0:offset).limit((!limit)?50:limit).sort({ updateAt: -1 }).exec()
